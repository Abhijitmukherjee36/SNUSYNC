...
export default function AttendancePage() {
  ...
  const [histChip, setHistChip] = useState('All');   // All | High | Low | Perfect
  const [histFrom, setHistFrom] = useState('');      // date input yyyy-mm-dd
  ...
  const filteredHistory = useMemo(() => {
    return HISTORY.filter(h => {
      ...
      // date range filter — parse "D Mon YYYY" from HISTORY
      if (histFrom || histTo) {
        ...
        <div className="att-content">
          ...
          <div className="att-right">
            ...
            <div className="att-hist-filter">
              ...
              <input
                type="date" className="att-hist-date-input"
                title="From date"
              ...
              {(histChip !== 'All' || histFrom || histTo) && (
                <button className="att-hist-clear" onClick={() => { setHistChip('All'); setHistFrom(''); setHistTo(''); }}>Clear</button>
              )}
                ...
                <table className="att-history-table">
                  ...
                  <tbody>
                    {filteredHistory.length > 0 ? filteredHistory.map((h, i) => (
                      ...
                      <tr><td colSpan={5} style={{textAlign:'center',padding:'20px',color:'#bbb',fontFamily:'Inter,sans-serif',fontSize:12}}>No records match the filter.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
